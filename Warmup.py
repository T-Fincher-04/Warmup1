...
class MyApp(ShowBase):
    ...
    def __init__(self):
        ...
        self.accept('arrow_forward', self.positiveZ, [1])
    ...
    def negativeZ(self, keyDown):
        if keyDown:
            self.taskMgr.add(self.moveNegativeZ, 'moveNegativeZ')
        else:
            self.taskMgr.remove('moveNegativeZ')

    def positiveZ(self, keyDown):
        if keyDown:
            self.taskMgr.add(self.movePositiveZ, 'movePositiveZ')
        else:
            self.taskMgr.remove('movePositiveZ')
    ...
    def moveNegativeZ(self, task):
        self.fighter.setZ(self.fighter, -1)
        return task.cont

    def movePositiveZ(self, task):
        self.fighter.setZ(self.fighter, 1)
        return task.cont
